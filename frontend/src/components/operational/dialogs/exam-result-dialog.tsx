...
// TODO: migrate to User from @/types when component is refactored
interface Usuario {
  id: string;
  nome: string;
...
type ResultadoProva = 'Aprovado' | 'Reprovado' | 'No Show' | '';
...
interface ExamResultDialogProps {
  ...
  onRegistrarResultado: (alunoId: string, resultado: 'Aprovado' | 'Reprovado' | 'No Show', observacoes: string, usuarioId: string) => void;
}

export const ExamResultDialog: React.FC<ExamResultDialogProps> = ({
  open,
  onOpenChange,
  aluno,
  prova,
  ...
  const [resultado, setResultado] = useState<ResultadoProva>('');
  ...
  const handleConfirmar = () => {
    ...
    // Registrar resultado
    onRegistrarResultado(aluno.id, resultado as 'Aprovado' | 'Reprovado' | 'No Show', observacoes, usuarioAtual.id);
    ...
    toast.success(mensagens[resultado as keyof typeof mensagens]);
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <Select value={resultado} onValueChange={(value: ResultadoProva) => setResultado(value)}>
