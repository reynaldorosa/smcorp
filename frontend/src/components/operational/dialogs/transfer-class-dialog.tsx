...
// TODO: migrate to Class, Course, Room, Instructor, CostEntry from @/types
interface Turma {
  id: string;
  cursoId: string;
  codigo: string;
...
interface Sala { id: string; nome: string; localizacao: string; }
interface LancamentoCusto { id: string; codigo: string; valor: number; observacoes?: string; }
interface Instrutor { id: string; }
...
interface TransferClassDialogProps {
  ...
  onTransferirAluno: (alunoId: string, novaTurmaId: string) => void;
  verificarCustosProvaParaExcluir?: (alunoId: string) => { custos: LancamentoCusto[]; excluir: boolean; motivo: string } | null;
}
...
export const TransferClassDialog: React.FC<TransferClassDialogProps> = ({
  ...
  lancamentosCusto = [],
  onTransferirAluno,
  verificarCustosProvaParaExcluir
}) => {
  const [turmaSelecionada, setTurmaSelecionada] = useState<string | null>(null);
  const [verificacaoCustos, setVerificacaoCustos] = useState<{ custos: LancamentoCusto[]; excluir: boolean; motivo: string } | null>(null);

  // Verificar custos quando o dialog abre
  useEffect(() => {
    if (open && verificarCustosProvaParaExcluir) {
      const result = verificarCustosProvaParaExcluir(aluno.id);
      setVerificacaoCustos(result);
    }
  }, [open, aluno.id, verificarCustosProvaParaExcluir]);
  ...
  const handleTransferir = () => {
    ...
    onTransferirAluno(aluno.id, turmaSelecionada);
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  ...
                  {formatDate(turmaAtual.dataInicio)} - {formatDate(turmaAtual.dataFim)}
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-600">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="text-xs text-gray-500">
                              Preço: R$ {Number(turma.preco || 0).toFixed(2)}
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              <div className="bg-white rounded border border-yellow-300 p-3 space-y-2">
                ...
                <div className="mt-2 pt-2 border-t border-yellow-200 flex justify-between items-center">
                  ...
                  <span className="text-sm font-bold text-red-700">
                    R$ {verificacaoCustos.custos.reduce((sum, c) => sum + Number(c.valor), 0).toFixed(2)}
                  </span>
                </div>
              </div>
            </div>
