...
interface DialogSelecionarSubstitutoProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  alunoAntigo: Aluno;
  alunosFilaEspera: Aluno[];
  onConfirmar: (alunoNovoId: string, motivo: string) => void;
}

export const DialogSelecionarSubstituto: React.FC<DialogSelecionarSubstitutoProps> = ({
  open,
  onOpenChange,
  alunoAntigo,
  alunosFilaEspera,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Substituindo: <strong>{alunoAntigo.nome}</strong> ({alunoAntigo.codigoSistema})
          </DialogDescription>
        </DialogHeader>
            ...
            <>
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <div className="text-sm text-red-800 space-y-1">
                  <p><strong>Nome:</strong> {alunoAntigo.nome}</p>
                  <p><strong>Código:</strong> {alunoAntigo.codigoSistema}</p>
                  <p><strong>CPF:</strong> {alunoAntigo.cpf}</p>
                  <p><strong>Telefone:</strong> {alunoAntigo.telefone}</p>
                </div>
              </div>
