...
import { Aluno } from '@/app/contexts/SMCorpContext';
...
interface DialogPagamentoProps {
  ...
  clientesPJ?: Array<{ id: string; nome: string; cnpj: string; formasPagamentoPermitidas?: string[] }>;
}

export const DialogPagamento: React.FC<DialogPagamentoProps> = ({
  open,
  onOpenChange,
  aluno,
  ...
  const handleRegistrar = () => {
    ...
    // 🔧 BOLETO: Código de barras e data de vencimento são OPCIONAIS no registro
    // O Master deve preencher esses dados ao confirmar o recebimento
  ...
  const handleConfirmarComDadosBoleto = (pagamentoId: string) => {
    ...
    onEditarPagamento(pagamentoId, {
      valor: aluno.pagamentos!.historico.find(p => p.id === pagamentoId)!.valor,
      formaPagamento: 'Boleto',
      observacoes: aluno.pagamentos!.historico.find(p => p.id === pagamentoId)!.observacoes || '',
  ...
  const getNomeEmpresa = () => {
    if (aluno.tipoPessoa === 'PJ' && aluno.clientePJId && clientesPJ) {
      ...
      return empresa ? empresa.nome : 'Empresa';
    }
    return null;
  };
  ...
  const gerarReciboPagamento = (pagamento: any) => {
    const nomeEmpresa = getNomeEmpresa();
    const vinculadoA = nomeEmpresa || `CPF ${aluno.cpf}`;
    ...
    const produtosExtras = aluno.produtosExtras && aluno.produtosExtras.length > 0 
      ? aluno.produtosExtras.map((p: any) => p.nome).join(', ')
      : '';
                        ...
                        {pagamento.formaPagamento === 'Boleto' && pagamento.codigoBarrasBoleto && (
                          <div className="bg-yellow-50 border border-yellow-200 p-2 rounded mb-2 text-xs">
                            ...
                            <div className="text-gray-700">
                              <strong>Vinculado a:</strong> {pagamento.vinculadoA || (aluno.tipoPessoa === 'PJ' && aluno.clientePJId ? getNomeEmpresa() || 'CNPJ da empresa' : `CPF ${aluno.cpf}`)}
                            </div>
                          </div>
                        )}
                        ...
                        {!pagamento.confirmedoPor && usuarioAtual.nivel === 'Master' && confirmandoBoletoId === pagamento.id && (
                          <div className="mt-3 bg-orange-50 border-2 border-orange-300 p-3 rounded-lg space-y-3">
                            ...
                            <div className="text-xs text-gray-600 bg-yellow-100 p-2 rounded">
                              <strong>Vinculado a:</strong> {aluno.tipoPessoa === 'PJ' && aluno.clientePJId ? 'CNPJ da empresa' : `CPF ${aluno.cpf}`}
                            </div>
